...
/*
 * Extract bit segments of specified width from 8 padded FPx values.
 *
 * This function extracts bit segments from 8 FPx values that have been padded
 * (each FPx value stored in one byte with bits in the high positions).
 * The extracted segments are packed into the output array.
 *
 * Parameters:
 *   Seg_xbit     - Output: Array to store extracted bit segments, size = bit_width bytes
 *   Padded_8_FPx - Input: 8 padded FPx values (each in 1 byte, value in high bits)
 *   bit_width    - Number of bits to extract from each FPx value (1, 2, or 4)
 *   bit_offset   - Bit position offset for extraction within each padded byte
 *
 * Usage:
 *   For FP6 (BIT_WIDTH=6), this extracts 1-bit, 2-bit, or 4-bit segments
 *   that will be stored separately for optimized GPU memory access.
 */
...
/*
 * Assign 32 FPx values to 4 threads, splitting each value into 1/2/4-bit segments.
 *
 * This function processes 4 blocks of 8 FPx values (32 total), extracts each FPx
 * from the packed storage, and splits them into separate bit segments (1-bit, 2-bit,
 * and 4-bit) for each of the 4 threads. This segmentation enables efficient
 * parallel dequantization on the GPU.
 *
 * Template Parameters:
 *   EXPONENT - Number of exponent bits in the FPx format
 *   MANTISSA - Number of mantissa bits in the FPx format
 *              (Total bit width = 1 + EXPONENT + MANTISSA)
 *
 * Parameters:
 *   Vec_Seg_1bit - Output: Array of 4 vectors to store 1-bit segments for each thread
 *   Vec_Seg_2bit - Output: Array of 4 vectors to store 2-bit segments for each thread
 *   Vec_Seg_4bit - Output: Array of 4 vectors to store 4-bit segments for each thread
 *   PTR          - Input: Array of 4 pointers to source data blocks
 *
 * Note:
 *   Which segment arrays are populated depends on BIT_WIDTH:
 *   - If BIT_WIDTH & 1: 1-bit segments are used
 *   - If BIT_WIDTH & 2: 2-bit segments are used
 *   - If BIT_WIDTH & 4: 4-bit segments are used
 *   For FP6 (BIT_WIDTH=6): uses 2-bit and 4-bit segments
 */
...
/*
 * Perform bit-level interleaving on 4 bytes of data for optimized GPU access.
 *
 * This function rearranges bits within a 4-byte word according to a predefined
 * interleaving pattern. This optimization ensures that when 32 GPU threads
 * access the data, memory coalescing is maximized and bank conflicts are minimized.
 *
 * Template Parameters:
 *   BIT_WIDTH - The bit width of each element (1, 2, or 4 bits)
 *
 * Parameters:
 *   PTR_4Bytes - Input/Output: Pointer to 4 bytes to be interleaved in-place
 *
 * Interleaving Patterns (pre-defined for FP6-LLM):
 *   1-bit: 32 elements reordered by order_1bit array
 *   2-bit: 16 elements reordered by order_2bit array
 *   4-bit: 8 elements reordered by order_4bit array
 *
 * Purpose:
 *   Ensures efficient memory access patterns during GPU kernel execution
 *   by aligning data layout with the thread access pattern.
 */
...
/*
 * Prepack FPx weight matrix for efficient GPU kernel execution.
 *
 * This is the main prepacking function that transforms a raw FPx weight matrix
 * into an optimized layout for the FP6-LLM CUDA kernel. The transformation
 * involves three passes to ensure efficient memory access patterns.
 *
 * Template Parameters:
 *   EXPONENT - Number of exponent bits in the FPx format
 *   MANTISSA - Number of mantissa bits in the FPx format
 *
 * Parameters:
 *   packed_weights - Output: Prepacked weight buffer, laid out as:
 *                    [1-bit segment (if used)] + [2-bit segment (if used)] + [4-bit segment (if used)]
 *                    Total size: M * K * BIT_WIDTH / 8 bytes
 *   FPxWeights     - Input: Raw FPx weights stored continuously, size M * K * BIT_WIDTH / 8 bytes
 *   M              - Number of rows (must be multiple of 64)
 *   K              - Number of columns (must be multiple of 64)
 *
 * Processing Passes:
 *   Pass-1: Split weights into 1/2/4-bit segments and distribute to 32 threads
 *           Each 64x64 tile is processed, assigning weights to threads for parallel access
 *   Pass-2: Optimize for coalesced global memory access
 *           Reorder bytes so each warp can load 128 bytes efficiently
 *   Pass-3: Bit-level interleaving within each 4-byte word
 *           Final optimization for register-level access patterns
 *
 * Memory Layout After Prepacking:
 *   Data is organized so that 32 threads in a warp can cooperatively load
 *   and dequantize weights with minimal memory transactions.
 */
template<int EXPONENT, int MANTISSA>
...
/*
 * Prepack FP6 (E3M2) weight matrix for GPU kernel execution.
 *
 * Convenience wrapper for weight_matrix_prepacking_x_bit with FP6 format
 * (3 exponent bits + 2 mantissa bits + 1 sign bit = 6 bits total).
 *
 * Parameters:
 *   packed_weights - Output: Prepacked weights optimized for GPU access
 *   FP6Weights     - Input: Raw FP6 weights in continuous storage
 *   M              - Number of rows (must be multiple of 64)
 *   K              - Number of columns (must be multiple of 64)
 */
...
/*
 * Prepack FPx weight matrix with runtime-specified exponent and mantissa bits.
 *
 * This function provides runtime selection of the FPx format, dispatching
 ...
 *   packed_weights - Output: Prepacked weights optimized for GPU access
 *   FPxWeights     - Input: Raw FPx weights in continuous storage
 *   M              - Number of rows (must be multiple of 64)
 *   K              - Number of columns (must be multiple of 64)
 *
 * Supported Formats:
