...
/*
 * Computes DualTrits-FP16 GEMM (C++ interface).
 * 
 * DualTrits: 4-bit base-3 encoding representing 9 values.
 * Storage: 16 uint16_t per 64 elements (13 valid packs + 3 padding)
 */
...
/*
 * DualTrits weight prepacking and dequantization (C++ interface).
 * Uses padded format: 16 uint16_t per 64 elements for coalesced GPU access.
 */
void weight_prepacking_fp16_to_dual_trits_gpu(half* weight_16bit, uint16_t* weight_packed, size_t M, size_t K, half* scale);
void DeQuantMatrix_DualTrit5_Padded_To_FP16(half* A_16bit_h, uint16_t* A_packed_h, size_t M, size_t K, half* scale);
