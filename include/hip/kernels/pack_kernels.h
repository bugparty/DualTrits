#ifndef PROJECT_FLOAT_HIP_KERNELS_H
#define PROJECT_FLOAT_HIP_KERNELS_H

// HIP/ROCm kernel implementations for dual-trit packing.
// Mirrors include/cuda/kernels/pack_kernels.cuh; HIP provides the same
// device intrinsics (__umulhi, __umul64hi, __device__, __global__, etc.).
...
__global__ void pack_kernel_hip(DualTrits const* __restrict__ d_input, UInt* __restrict__ d_output, int outputSize) {
...
// Kernel: unpack batch of packed integers (stride loop)
template <std::size_t TritsPerPack, class UInt>
__global__ void unpack_kernel_stride_hip(UInt const* __restrict__ d_input, DualTrits* __restrict__ d_output, const int n) {
...
// Kernel: unpack batch of packed integers (simple)
template <std::size_t TritsPerPack, class UInt>
__global__ void unpack_kernel_hip(UInt const* d_input, DualTrits* d_output, int n) {
    const int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid < n) {
...
#endif // PROJECT_FLOAT_HIP_KERNELS_H
